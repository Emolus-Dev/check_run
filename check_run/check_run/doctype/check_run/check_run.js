      ...
      .done((r) => {
        if (!r.message.ach_only) {
          ...
          if (frm.doc.status == 'Ready to Print') {
            frm.add_custom_button('Download Checks', () => {
              download_checks(frm);
            });
          }
        }
        if (!r.message.print_checks_only) {
          if (frm.doc.docstatus == 1) {
            frm.add_custom_button('Download NACHA File', () => {
              download_nacha(frm);
            });
          }
        }
...
function confirm_print(frm) {
    ...
    fields: [
      {
        fieldname: 'ht',
        fieldtype: 'HTML',
        options: `<button id="confirm-print" class="btn btn-sm btn-success" style="width: 48%">Confirm Print</button>
  ...
  d.wrapper.find('#confirm-print').on('click', () => {
    frappe
      .xcall('check_run.check_run.doctype.check_run.check_run.confirm_print', { docname: frm.doc.name })
      .then(() => {
        d.hide();
        frm.reload_doc();
      });
    ...
    fields: [
      {
        fieldname: 'ht',
        fieldtype: 'HTML',
        options: `<button id="reprint" class="btn btn-sm btn-warning" style="width: 48%; color: white;">Re-Print Checks</button><br><br>`,
      },
      {
        fieldname: 'reprint_check_number',
        fieldtype: 'Data',
...
function download_checks(frm) {
  frappe
    .xcall('check_run.check_run.doctype.check_run.check_run.download_checks', { docname: frm.doc.name })
    .then((r) => {
      if (r) {
        frm.reload_doc();
        window.open(r);
...
function get_acc_bank_currency(frm) {
  frappe
    .xcall('check_run.check_run.doctype.check_run.check_run.get_bank_account_currency', {
      bank_account: frm.doc.bank_account,
    })
    .then((r) => {
      if (r) {
        frm.set_value('bank_account_currency', r);
        frm.refresh_field('bank_account_currency');
      }
    });
}
